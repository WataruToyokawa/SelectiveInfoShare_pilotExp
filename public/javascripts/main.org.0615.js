...
const instructionText_indiv = 
	[ 'Please read the following instructions carefully. After reading the instructions, we will ask a few questions to verify your understanding of the experimental task. <br><br>After answering these questions, you may spend some time in a waiting room until sufficient number of participants have arrived to start the task. <br><br>You will be paid <span class="note">13.2 cents per minute</span> ($8 per hour pro rata) for any time spent in the waiting room. When all participants are ready, the main task will start.'
	...
	, '<br><br>The total reward you get will be converted into real money. The exchange rate is <span class="note">500&nbsp;points = 10&nbsp;US cent.</span>'

	, '<br>The reward for each slot is stochastic, but <span class="note">some of the slots may generate a higher payoff (on average) than the other</span>. The expected payoff of the slot is the same from trial to trial.'

	, '<br><br><br><br>From the next page, you will play a tutorial to get familiar with the task!'
	];
const tutorialText_indiv = 
	[ '<br>This is the tutorial task. <br><br>First, choose whichever you like!'
	...
	, 'You have <span class="note">up to 15 seconds</span> in making a choice. <br><br>If time is up before making your choice, <span class="note">no reward</span> drops for the trial. Let\'s see what will happen if your time is up...'

	, '<br><br>Time was up and you missed the trial! You got nothing.'

	, '<br>The tutorial is done. <br><br>Next, you will proceed to a short understanding quiz!'
	];
const understandingCheckText_indiv = 
	...
	, 'Is it possible and allowed to choose the same option repeatedly?' //YES

	, 'Does your bonus of this HIT increase as getting more reward points from the task?' //YES
	];

const instructionText_group = 
	[ 'Please read the following instructions carefully. After reading the instructions, we will ask a few questions to verify your understanding of the experimental task. <br><br>After answering these questions, you may spend some time in a waiting room until sufficient number of participants have arrived to start the task. <br><br>You will be paid <span class="note">13.2 cents per minute</span> ($8 per hour pro rata) for any time spent in the waiting room. When your group is ready, the main task will start.'
...
const revisitingInstructionText = 
	...
	, '<br><br>That\'s it! Take the understanding quiz again in the next page.'
	]
...
window.onload = function() {
    ...
    , surePositionX = 200
    , riskyPositionX = 600
    , missPositionX = (surePositionX+riskyPositionX)/2
    , surePosition = 0
    , riskyPosition = 1
	...
	class SceneTutorial extends Phaser.Scene {
		...
		create(){
		    ...
		    if (tutorialTrial == 1) {
		    	...
		    	if (choice_tutorial == 1) payoffText.x = surePositionX;
		    	if (choice_tutorial == 2) payoffText.x = riskyPositionX;
		    ...
		    if (indivOrGroup == 1 & tutorialTrial == 1) {
			    socialFreqNumbers.sure = this.add.text(surePositionX, socialInfoY, ``, { fontSize: '25px', fill: noteColor }).setOrigin(0.5,0.5);
			    socialFreqNumbers.risky = this.add.text(riskyPositionX, socialInfoY, ``, { fontSize: '25px', fill: noteColor }).setOrigin(0.5,0.5);
			    numberOfPreviousChoice_sure = 0;
			    numberOfPreviousChoice_risky = 0;
			    socialFreqNumbers.sure.visible = false;
			    socialFreqNumbers.risky.visible = false;
			} else if (indivOrGroup == 1 & tutorialTrial == 2) {
			    socialFreqNumbers.sure = this.add.text(surePositionX, socialInfoY, `2 people chose`, { fontSize: '25px', fill: noteColor }).setOrigin(0.5,0.5);
			    socialFreqNumbers.risky = this.add.text(riskyPositionX, socialInfoY, `3 people chose`, { fontSize: '25px', fill: noteColor }).setOrigin(0.5,0.5);
			    ...
			    showStars.call(this, numberOfPreviousChoice_sure, numberOfPreviousChoice_risky, socialInfoY);
			} else if (indivOrGroup == 1 & tutorialTrial == 3) {
			    socialFreqNumbers.sure = this.add.text(surePositionX, socialInfoY, `3 people chose`, { fontSize: '25px', fill: noteColor }).setOrigin(0.5,0.5);
			    socialFreqNumbers.risky = this.add.text(riskyPositionX, socialInfoY, `2 people chose`, { fontSize: '25px', fill: noteColor }).setOrigin(0.5,0.5);
			    numberOfPreviousChoice_sure = 3; // 3
			    numberOfPreviousChoice_risky = 2; // 2
			    // function.call(what_is_this, ...) method can specify what you mean by "this" in the function
			    showStars.call(this, numberOfPreviousChoice_sure, numberOfPreviousChoice_risky, socialInfoY);
			} else if (tutorialTrial != 4) {
				socialFreqNumbers.sure = this.add.text(surePositionX, socialInfoY, `You chose this`, { fontSize: '25px', fill: noteColor }).setOrigin(0.5,0.5);
			    socialFreqNumbers.risky = this.add.text(riskyPositionX, socialInfoY, `You chose this`, { fontSize: '25px', fill: noteColor }).setOrigin(0.5,0.5);
			    socialFreqNumbers.sure.visible = false;
			    socialFreqNumbers.risky.visible = false;
			    if (choice_tutorial == 1) {
			    	socialFreqNumbers.sure.visible = true;
			    ...
			    if (choice_tutorial == 2) {
			    	socialFreqNumbers.sure.visible = false;
			    	numberOfPreviousChoice_sure = 0;
			    	socialFreqNumbers.risky.visible = true;
	...
	class SceneMain extends Phaser.Scene {
		...
		create(){
			...
			// max time in the choice stage
			/*countDownChoiceStage = setTimeout(function(){
				//console.log('maxChoiceStageTime = '+maxChoiceStageTime);
                isChoiceMade = true;
                currentChoiceFlag = -1
                objects.box1.visible = false;
				objects.box1_active.visible = false;
                objects.box2.visible = false;
				objects.box2_active.visible = false;
				madeChoice(currentChoiceFlag, 'miss');
		    ...
		    objects.box1_active.on('pointerdown', function (pointer) {
		    	...
		    	if(!isChoiceMade) {
		    		madeChoice(currentChoiceFlag, exp_condition);
		    		gameTimer.destroy();
		    ...
		    let socialFreqNumbers = {};
		    if (indivOrGroup == 1) {
			    socialFreqNumbers.sure = this.add.text(surePositionX, slotY_main+100, `${mySocialInfoList['sure']} people chose`, { fontSize: '25px', fill: noteColor }).setOrigin(0.5,0.5);
			    socialFreqNumbers.risky = this.add.text(riskyPositionX, slotY_main+100, `${mySocialInfoList['risky']} people chose`, { fontSize: '25px', fill: noteColor }).setOrigin(0.5,0.5);
			} else {
				socialFreqNumbers.sure = this.add.text(surePositionX, slotY_main+100, `You chose this`, { fontSize: '25px', fill: noteColor }).setOrigin(0.5,0.5);
			    socialFreqNumbers.risky = this.add.text(riskyPositionX, slotY_main+100, `You chose this`, { fontSize: '25px', fill: noteColor }).setOrigin(0.5,0.5);
			    socialFreqNumbers.sure.visible = false;
			    socialFreqNumbers.risky.visible = false;
		    ...
		    ;
		    showStars.call(this, numberOfPreviousChoice_sure, numberOfPreviousChoice_risky, slotY_main+110);
    ...
    function showStars (num_sure, num_risky, socialInfoY) {
    	...
    	if (quotient_num_sure > 0) {
    		for (let q=0; q<quotient_num_sure; q++) {
	    		stars_sure[q] = this.add.group({
			        ...
			        repeat: 5-1, 
			        setXY: { x: surePositionX_new - 20*q, y: socialInfoY+20, stepY: 15 }
			    });
	    	}
    	}
    	...
    	if (mod_num_sure > 0) {
    		stars_sure[quotient_num_sure] = this.add.group({
		        ...
		        setXY: { x: surePositionX_new - 20*quotient_num_sure, y: socialInfoY+20, stepY: 15 }
		    });
    	}
		    ...
		    /*stars_sure = this.add.group({
		        ...
		        // which is just what we need for our game:
		        repeat: num_sure-1, 
		        setXY: { x: surePositionX, y: socialInfoY+20, stepY: 15 }
		    });*/
		//}
		/*if (num_risky>0) {
		    stars_risky = this.add.group({
		        ...
		        // which is just what we need for our game:
		        repeat: num_risky-1, 
		        setXY: { x: riskyPositionX, y: socialInfoY+15, stepY: 15 }
		    });
		}*/
		//}
		let mod_num_risky = num_risky % 5;
    	let quotient_num_risky = Math.floor(num_risky / 5);
    	let riskyPositionX_new = riskyPositionX-15 + 10*quotient_num_risky
    	// First, draw 5 stars
    	if (quotient_num_risky > 0) {
    		for (let q=0; q<quotient_num_risky; q++) {
	    		stars_risky[q] = this.add.group({
			        ...
			        // which is just what we need for our game:
			        repeat: 5-1, 
			        setXY: { x: riskyPositionX_new - 20*q, y: socialInfoY+20, stepY: 15 }
			    });
	    	}
    	}
    	// Then, draw the remaining stars
    	if (mod_num_risky > 0) {
    		stars_risky[quotient_num_risky] = this.add.group({
		        ...
		        setXY: { x: riskyPositionX_new - 20*quotient_num_risky, y: socialInfoY+20, stepY: 15 }
		    });
    	}
    }   

    // madeChoice
    function madeChoice (flag, distribution) {  
        let thisChoice;
        if (flag == -1) {
			thisChoice = 'miss';
			payoffText.x = 400;
    ...
    function randomChoiceFromTwo(choice, arr, p_rare, socialInfo, publicInfo) {
    	...
    	if (p_rare < roulette) { // common event
    		thisPayoff = Math.floor((arr[0] + noise)*100);
    		myEarnings.push(thisPayoff);
            myChoices.push(choice);   		
    	} else { // rare event
    		thisPayoff = Math.floor((arr[1] + noise)*100);
    		myEarnings.push(thisPayoff);
            myChoices.push(choice);
    	}
